...
INPUT_VIDEO = r"E:\Evoq\Sprinting video\IMG_0772.MOV"  # <--- your path

# Choose ONE selection method:
SELECTION_METHOD = "auto"    # "roi" or "class"
INIT_FRAME_INDEX = 0      # frame index to show for ROI and to start tracking
...
def main_yolo_keep_one_object() -> Dict[str, Any]:
    ...
    if SELECTION_METHOD == "class":
        ...
        # Automatically pick the "person" class with highest confidence in the init frame

        wanted_id = name_to_class_id.get("person", None)

        if wanted_id is None:
            raise RuntimeError("Model does not include 'person' class.")

        candidates = [ins for ins in init_instances if ins["cls"] == wanted_id]

        if not candidates:
            raise RuntimeError("No 'person' detected on initialization frame.")
        ...
        target_id_text = f"Auto-selected 'person' (conf={best['conf']:.2f})"

        print(f"[INFO] Auto-selected bounding box for person: {target_bbox}")

        # Compute smooth box variables
        ...
        print(f"[INFO] ROI automatically selected at center ({cx0:.1f}, {cy0:.1f}) with size ({w0}x{h0})")
...
def classify_pose(angle_dict, min_hits=4):
    ...
    Evaluates both right-leg–based and left-leg–based versions of each pose.
    Returns: ("PoseName (Right)" or "(Left)", confidence)
    """
    import numpy as np

    # Extract all relevant joint angles safely
    ...
    # --------------- DEFINE RULES (Right-foot version) -----------------
    rules_right = {
        "Toe Off": [
    ...
    # --------------- Mirror RULES for left-foot version -----------------
    rules_left = {}
    for pose, conds in rules_right.items():
        # Swap left/right angle variables in each rule
        mirrored = [
            conds[1], conds[0],  # knees
            conds[3], conds[2],  # hips
            conds[5], conds[4],  # elbows
            conds[7], conds[6],  # shoulders
            conds[9], conds[8],  # ankles
        ]
        rules_left[pose] = mirrored

    # ------------------- SCORE BOTH SIDES -------------------
    pose_scores_right = {pose: int(np.nansum(conds)) for pose, conds in rules_right.items()}
    pose_scores_left  = {pose: int(np.nansum(conds)) for pose, conds in rules_left.items()}

    best_right = max(pose_scores_right, key=pose_scores_right.get)
    best_left  = max(pose_scores_left,  key=pose_scores_left.get)

    right_score = pose_scores_right[best_right]
    left_score  = pose_scores_left[best_left]
    max_possible = len(rules_right[best_right])

    # Decide side
    if right_score >= left_score and right_score >= min_hits:
        conf = round((right_score / max_possible) * 100.0, 1)
        return f"{best_right} (Right)", conf
    elif left_score > right_score and left_score >= min_hits:
        conf = round((left_score / max_possible) * 100.0, 1)
        return f"{best_left} (Left)", conf
    else:
...
def export_best_pose_frames(classified_csv,
    ...
    # change bumber 01 bla bla bla
    for pose_name in poses:
        # accept both "Pose" and "Pose (Left/Right)" labels
